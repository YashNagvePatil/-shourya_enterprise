...
import { usePayment } from "../hook/usePayment"; // Apne hook ka exact path dein

const PaymentPage = ({ orderData }) => {
  const { executePayment, loading, error } = usePayment();
  const [selectedMethod, setSelectedMethod] = useState("card");

  // Dummy order fallbackagar props na milein
  const order = orderData || {
    _id: "ORD67890123",
    items: [
      { name: "Executive Health Bundle", qty: 1, price: 2499 },
      { name: "Wellness Booster Pack", qty: 2, price: 999 },
    ],
    totalPrice: 4497,
  ...
  const handlePayment = async () => {
    const result = await executePayment(order._id);
    if (result.success) {
      // Success hone par receipt page par redirect karein
      window.location.href = `/receipt/${result.data?.orderId || order._id}`;
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80">
              <h2 className="text-lg font-bold text-slate-800 mb-3 flex items-center justify-between">
                Shipping Details
                <span className="text-xs text-blue-600 font-normal hover:underline cursor-pointer">Edit</span>
              </h2>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80">
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <input type="radio" checked={selectedMethod === "card"} readOnly className="accent-indigo-600" />
                </label>
                ...
                >
                  ...
                  <input type="radio" checked={selectedMethod === "upi"} readOnly className="accent-indigo-600" />
                </label>
                ...
                >
                  ...
                  <input type="radio" checked={selectedMethod === "netbanking"} readOnly className="accent-indigo-600" />
                </label>
              </div>
            </div>
          ...
          {/* Right Column: MLM Points & Price Summary */}
          <div className="lg:col-span-5 space-y-6">
            
            {/* MLM BV & PV Earnings Card */}
            <div className="bg-gradient-to-br from-indigo-900 via-indigo-800 to-purple-900 rounded-2xl p-5 text-white shadow-lg relative overflow-hidden">
              ...
              <div className="flex items-center gap-2 text-indigo-200 text-xs font-semibold tracking-wider uppercase mb-2">
                <Sparkles className="w-4 h-4 text-amber-400" /> Distributor Rewards
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/10 backdrop-blur-md p-3 rounded-xl border border-white/10">
                  <span className="text-xs text-indigo-200 block">Total BV Points</span>
                  <span className="text-2xl font-bold text-amber-300">{order.totalBV} BV</span>
                </div>
                <div className="bg-white/10 backdrop-blur-md p-3 rounded-xl border border-white/10">
                  <span className="text-xs text-indigo-200 block">Total PV Points</span>
                  <span className="text-2xl font-bold text-emerald-300">{order.totalPV} PV</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80 space-y-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between text-base font-bold text-slate-900 pt-2 border-t border-slate-100">
                  ...
                  <span className="text-indigo-600 text-xl">₹{order.totalPrice}</span>
                </div>
              </div>
              ...
              <button
                ...
                disabled={loading}
                className="w-full bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-semibold py-3.5 px-4 rounded-xl shadow-md shadow-indigo-200 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
